...
export class TrailerDeleteDialogComponent {
    ...
    constructor(private trailerService: TrailerService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class TrailerDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ trailer }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(TrailerDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
                this.ngbModalRef.componentInstance.trailer = trailer;
